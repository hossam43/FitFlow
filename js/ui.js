...
class UIManager {
    ...
    updateOverview() {
        ...
        if (caloriesProgress) {
            const percentage = Math.min((dailyData.calories / profile.dailyCalories) * 100, 100);
    ...
    renderHistory() {
        ...
        if (days.length === 0) {
            historyEntries.innerHTML = '<p class="empty-state">Your history will appear here as you track your fitness! 📊</p>';
            return;
        }

        historyEntries.innerHTML = days.map(day => {
